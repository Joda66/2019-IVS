...
ivs_simple_test <- ivs_simple_test %>% add_predictions(model = mod)
...
p + geom_line(aes(y = IVS, color = (year(Datum) == 2018))) +
  geom_line(aes(y = pred),
            linetype = 2, # dashed
            color = "red",
            data = ivs_simple_test)

# Performance-Metriken
fPerformance <- function(actual, predicted) {
...
fPerformance(ivs_simple_test$IVS, ivs_simple_test$pred)
...
<!-- ivs_simple_train <- ivs_simple_train %>% add_predictions(model = mod) -->

<!-- ggplot(ivs_simple_train, aes(x = Monat)) + -->
<!--   # geom_boxplot(aes(y = IVS - mean(IVS)), alpha = 0.2, color = "grey") + -->
<!--   geom_boxplot(aes(y = IVS - mean(IVS)), alpha = 0.2, color = "grey") + -->
<!--   # geom_point(aes(y = IVS - mean(IVS)), alpha = 0.5) + -->
<!--   # geom_point(aes(y = pred - mean(IVS)), color = "red", alpha = 0.5) + -->
<!--   labs(title = "monatliche Inverkehrssetzungen von Personenkraftwagen", -->
<!--        subtitle = "Schweiz, 2013 - 2018") -->
...
ivs_ts <- ts(ivs_simple_train$IVS, start = c(2013,1), frequency = 12)

ivs_ts %>% ggtsdisplay(lag.max = 60)
ivs_ts %>% 
  diff() %>% 
  diff(lag = 12) %>% 
  ggtsdisplay(lag.max = 60, main = "ivs_simple_train$IVS, two times differentiated")

mod <- sarima(ivs_ts, 1,1,0, 0,1,1, 12)
sarima(ivs_ts, 2,1,0, 0,1,1, 12)

ivs_ts %>% plot()
ivs_ts_all <- ts(ivs_simple$IVS, start = c(2013,1), frequency = 12)
ivs_ts_all %>% plot()

predictions <- sarima.for(ivs_ts, 2,1,0, 0,1,1, 12, n.ahead = 12)
# lines(ivs_ts_all)

fPerformance(ivs_simple_test$IVS, predictions$pred)
```
