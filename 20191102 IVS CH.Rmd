...
p <- ivs_simple %>% # Diagrammgrunddaten festlegen
  ...
  scale_x_date(
    ...
    limits = c(ymd("2013-1-31", "2018-12-31")),
    date_labels = "%Y",
...
# mod <- lm(formula = IVS ~ 0 + Datum + Monat, data = ivs_simple_train)
...
autoplot(fc) + 
  ...
  scale_x_continuous( # Formatierung der x-Achse
    name = "",
    minor_breaks = seq(2013, 2019, by = 0.25)
  ) +
...
p <- ivs_fuel %>%
  ...
  scale_x_date(
    name = "",
    limits = c(ymd("2013-01-31", "2018-12-31")),
    date_labels = "%m/%y",
    date_minor_breaks = "3 months",
    date_breaks = "years"
  )
...
ivs_train_noLog %>% 
  ...
  scale_y_log10(name = "Inverkehrssetzungen pro Monat",
                labels = formatT) + # Logarithmische Ordinate
  facet_wrap( ~ Treibstoff) + # kleine Subdiagramm je Treibstoffart.
  labs(title = "Inverkehrssetzungen von Personenkraftwagen nach Treibstoffart",
       subtitle = "Schweiz, 2013 - 2018", x = NULL)
...
ivs_test_noLog <- ivs_test[ivs_test$IVS > 0 & ivs_test$pred > 0,]
ivs_test_noLog %>% # bitte keine Nullen, die logarithmiert werden ...
  ggplot(aes(x = Datum)) +
...
```{r}
  ...
  ggplot(ivs_train_noLog, aes(x = Datum)) + # Datum auf der x-Achsel
  geom_line(aes(y = IVS), color = "black") + # tatsächliche Werte, Training
  geom_line(data = ivs_test_noLog,  # tatsächliche Werte, Test
            aes(y = IVS), color = "blue") +
  geom_line(data = ivs_test_noLog, # Vorhersagewerte
            aes(y = pred), color = "red") + 
  scale_y_log10(name = "Inverkehrssetzungen pro Monat",
                labels = formatT) + # Logarithmische Ordinate
  facet_wrap( ~ Treibstoff) + # kleine Subdiagramm je Treibstoffart.
  labs(title = "Anpassung und Prognose im Regressionsmodell",
       subtitle = "Trainingsdaten (schwarz), Testdaten (blau), Prognosen (rot)", x = NULL)
...
ivs_test_metrics %>% select(-data, -model) %>% arrange(mape) 
